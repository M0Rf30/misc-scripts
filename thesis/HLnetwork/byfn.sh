...
function replacePrivateKey () {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/acme.iot.net/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# for the sample network. There are three members - one Orderer Org (``Orderer``)
# and one Peer Orgs (``Acme``) managing and maintaining two peer nodes.
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for acmeMSP   ##########"
  echo "#################################################################"
  configtxgen -profile OneOrgChannel -outputAnchorPeersUpdate ./channel-artifacts/acmeMSPanchors.tx -channelID $CHANNEL_NAME -asOrg acmeMSP
  if [ "$?" -ne 0 ]; then
